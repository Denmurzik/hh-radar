...
def render_page(
    *,
    ...
    # Всё, что утверждает страница о происхождении данных, зависит от одного
    # признака. Заголовок «что происходит на рынке» и подпись «данные получены
    # через API hh» на демонстрационной базе были бы неправдой, а неправда в
    # подвале обесценивает и те цифры, которые посчитаны честно.
    demo = vacancies_total < MIN_MEANINGFUL_VACANCIES
    ...
    page_title = (
        "hh-radar — витрина на демонстрационных данных"
        if demo
        else "hh-radar — срез рынка AI и автоматизации"
    )
    heading = (
        "Так выглядит витрина hh-radar" if demo else "Что происходит на рынке AI и автоматизации"
    )
    lede = (
        "Страница собрана программой из демонстрационного корпуса — тем же кодом и "
        "теми же запросами, которыми она собирается из настоящей базы. Ни одна цифра "
        "здесь не вписана руками, но и о рынке они не говорят: смотрите на устройство, "
        "а не на числа."
        if demo
        else "Страница собрана программой из собственной базы вакансий hh.ru — той же, "
        "которую LLM-агент опрашивает через MCP. Ни одна цифра здесь не вписана руками."
    )
    origin_note = (
        "Данные на этой странице — демонстрационные: вакансии и работодатели вымышлены "
        "и сгенерированы скриптом из репозитория. Настоящая база собирается через "
        "официальное API hh.ru с токеном приложения, хранится локально и показывается "
        "только агрегатами — чужие объявления в публичном репозитории не лежат."
        if demo
        else "Данные получены через официальное API hh.ru с токеном приложения, хранятся "
        "локально и показаны здесь только агрегатами."
    )
...
<div class="wrap">
  ...
  <footer class="foot">
    ...
    вакансии руками, надоело, автоматизировал. {origin_note}</p>
...
def _sample_data_warning(vacancies_total: int) -> str:
    ...
    return (
        ...
        f"В базе {vacancies_total} "
        f"{plural_ru(vacancies_total, 'вакансия', 'вакансии', 'вакансий')} — "
        "столько кладёт команда <code>hh-radar seed</code>, и они "
        "<strong>вымышлены</strong>: вакансии, работодатели и зарплаты сгенерированы "
        "скриптом, потому что чужие объявления в публичном репозитории лежать не "
        "должны, а без токена приложения hh их и не получить. Цифры ниже посчитаны "
        "по этой базе честно, но о рынке они не говорят. Настоящий срез появляется "
        "после <code>hh-radar ingest</code> с токеном приложения.</p>"
    )
