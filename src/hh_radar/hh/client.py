        ...
        **extra: Any,
        ...
        if probe.found > MAX_RESULTS:
            ...
            if window <= MIN_WINDOW:
                ...
                # Обе половины включают саму границу: hh трактует date_from и
                # date_to включительно, и вакансия, опубликованная ровно в
                # середине, попадёт в обе. Это осознанно оставлено как есть —
                # запись идёт через UPSERT по идентификатору hh, поэтому
                # повтор превращается в обновление, а не в дубль. Сдвигать
                # границу на секунду значило бы рисковать пропуском.
